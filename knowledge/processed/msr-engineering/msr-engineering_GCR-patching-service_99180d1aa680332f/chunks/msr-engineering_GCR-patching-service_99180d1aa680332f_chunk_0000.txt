...
| <span style= "font-weight: normal; color: inherit;">SLO</span> | <span style="font-weight: normal;">Ensure 90% patch compliance</span>|
| --- | --- |
| SLI | [SLI Dashboard](https://gcrsandbox-hbd4erafhgcyeber.wus3.grafana.azure.com/d/_HM6ul14k/gcr-patching?orgId=1&refresh=1m) |
...
**SLO** - May need to be adjusted but is a swag of how successful the patching operations are.
**SLI** - Can use the existing or new dashboard for this.
**KVI** - Could get this table from the proposed SLI. 

## Introduction

We currently offer patching service for the nodes and Azure services that manages by us for GCR , SES and AKS and VMss patching. each link below takes you to the relevant documentation for each area.

- [SES-Patching](https://dev.azure.com/msresearch/MSR%20Engineering/_wiki/wikis/MSR-Engineering.wiki/8578/SES-Azure-update-manager-overview)
- [AKS Patching](https://dev.azure.com/msresearch/MSR%20Engineering/_wiki/wikis/MSR-Engineering.wiki/4738/AKS-Patching-service)
- [VMss Patching](https://dev.azure.com/msresearch/MSR%20Engineering/_wiki/wikis/MSR-Engineering.wiki/4739/VMss-Patching-service)

Now lets have an overview how Azure update management works.



SES is using the newest Azure update manager for its patching service offering. In this page we briefly explain how the Azure update manager works and what are the benefit of using it.
...
Use flexible patching options such as automatic virtual machine (VM) guest patching in Azure, hotpatching, and customer-defined maintenance schedules.
...
- Helps secure machines with new ways of patching, such as automatic VM guest patching in Azure, hot patching, or custom maintenance schedules.
...
<IMG  src="https://learn.microsoft.com/en-us/azure/update-manager/media/overview/update-management-center-overview.png"  alt="Diagram that shows the Update Manager workflow."/>
...
- Azure Arc-enabled servers agent for non-Azure Linux and Windows machines or physical servers.
...
All assessment information and update installation results are reported to Update Manager from the extension and is available
 for analysis with Azure Resource Graph. You can view up to the last seven days of assessment data, and up to the last 30 days of update installation results.
